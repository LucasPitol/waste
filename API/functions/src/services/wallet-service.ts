...
export class WalletService {
    ...
    async createNewWalletRes(userId: string, walletName: string) {
        ...
        var wallet = await this.walletSP.getWalletByNameAndOwnerId(walletName, userId)
    ...
    async removeMemberFromWalletResOld(memberId: string, walletId: string): Promise<ResponseDto> {
        let ret = new ResponseDto()
    ...
    async addMemberToWalletRes(memberId: string, walletId: string, ownerId: string): Promise<ResponseDto> {
        let ret = new ResponseDto()
    ...
    async removeMemberFromWalletRes(memberId: string, walletId: string, ownerId: string): Promise<ResponseDto> {
        let ret = new ResponseDto()
    ...
    async getWalletMembersRes(walletId: string, requesterId: string): Promise<ResponseDto> {
        let ret = new ResponseDto()
        ...
        if (!isOwner && !isMember) {
            ret.success = false
            ret.errorMsg = 'Acesso negado: você não tem permissão para ver os membros desta carteira'
            return ret
        }
