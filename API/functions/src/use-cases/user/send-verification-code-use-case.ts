...
export class SendVerificationCodeUseCase {
  ...
  async execute(userMail: string): Promise<ResponseDto> {
    ...
    var verificationCode = await this.verificationCodeService.getByUserMail(
      userMail
    );

    const now = new Date();
    const expirationDate =
      this.verificationCodeService.calculateExpirationDate(now);

    if (verificationCode == null) {
      this.verificationCodeService.save(userMail, code, expirationDate);
    } else if (
      this.verificationCodeService.canRegenerateCode(verificationCode, now)
    ) {
    ...
    await this.mailSenderService.sendVerificationCode(
      userMail,
      code,
      VerificationCodeType.newUser,
      expirationDate
